...
const kessStartupLog =
  window.__kessBoot?.startupLog ??
  function () {};
...
_flutter.loader.load({
  onEntrypointLoaded: async function (engineInitializer) {
    kessStartupLog('[startup] entrypoint_loaded at ' + performance.now().toFixed(1) + 'ms');
    const appRunner = await engineInitializer.initializeEngine();
    kessStartupLog('[startup] engine_initialized at ' + performance.now().toFixed(1) + 'ms');
